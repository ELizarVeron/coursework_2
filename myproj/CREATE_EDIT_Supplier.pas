...
constructor TForm13.Create(AOwner: TComponent);
  ...
  procedure TForm13.Init(sup:TSupplier);
        begin
              ...
              if sup.Type_='ООО' then
              ComboBox_type.ItemIndex:=0
              else
              if sup.Type_='ОАО' then
              ComboBox_type.ItemIndex:=1
              else
              if sup.Type_='ИП' then
              ComboBox_type.ItemIndex:=2
              else
              if sup.Type_='СЗ' then
              ComboBox_type.ItemIndex:=3;
               ...
               if (modeEdit = True) then
               begin
                Label1.Caption:= 'Редактироваие данных';
                Button2.Caption:='Удалить';
               end

               else
               begin
                   Label1.Caption:= 'Добавление нового поставщика';
