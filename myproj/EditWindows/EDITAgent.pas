 ...
 procedure TForm5.SaveInfoFromForm;
  ...
  procedure TForm5.Button1Click(Sender: TObject); //тут нужно изменять в бд только те записи которые мы изменили
          begin
             ...
             save_me:=true;
             SaveInfoFromForm;  //данные с формы
             if  (change_logo) then     //данные изображения с формы
             begin
...
function TForm5.SaveIconInComp;        //сохраняет иконку в комп и возвращает ее путь
  begin
     ...
     begin
        ...
        var s:= copy(str, 2, length(str) - 1);     //удалили первую кавычку
        var x:=pos( '"' , s);
        s:=copy( s, 1, x-1 ) ;   //s имя точки
        for var i := 0 to  points.Count-1 do
        begin
...
procedure TForm5.Button3Click(Sender: TObject); //добавить точку  в список и в массив
begin
          CreatePoint(agent_after_change.points);
end;

procedure TForm5.Button6Click(Sender: TObject); //удаление точки
begin
...
procedure TForm5.Button4Click(Sender: TObject);       //просто загрузка картинки
...
procedure TForm5.Button5Click(Sender: TObject);   //удаление картинки
begin
    Image1.Picture:=nil;
