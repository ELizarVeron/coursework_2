...
uses
  ...
  Vcl.ExtCtrls,
  Vcl.Imaging.jpeg, Vcl.Imaging.pngimage, EDITAgent, HistoryRelizAgent,HistoryPriority, Vcl.Menus, TABLE_Agents,Agent_Class, INFOAgent;
       ...
       if (EWin.need_delete=true) then
         begin
          ...
          TAgent_Class.load_frames(panel,0,TAgent_Class.array_of_agents.Count-1);     //перезагрузка панели с фреймами после удаления 1 ого фрейма
                      ...
                      //если было изменено что то в агенте или в точках или в логотипе   и было закрыто серез сохранение
...
begin
          InputQuery('Выбор приоритета', 'Изменить приоритет на:',s);
           ...
           begin
            ...
            ShowMessage('Приоритет это чсло от 0 до 10');
end;
...
procedure TFrame3.AddData ;     //загрузка данным в элементы фрейма из агента привязаннного к фрейму
begin
