...
if (NOT (Form2.ADOQuery1.IsEmpty)) then ShowMessage('Логин уже используется')
else
 begin
  Form2.ADOQuery1.Close; // отключаем запрос, если он был включен
  Form2.ADOQuery1.SQL.Clear; //
  Form2.ADOQuery1.SQL.Add('INSERT INTO Users (Staff, Login, Password_hash) VALUES (:staff,:logi,:passw)');
...
begin
  ...
  then ShowMessage('Введите все поля') else
   begin
             if (Edit2.Text <> Edit3.Text ) then  Label6.Visible := True
             else
                      begin
                        Label6.Visible := False;
                        if (Length(Edit2.Text)<7)
                        then
                        ShowMessage('Пароль должен содержать не менее 7 символов')
                        else Save_Acc(My_f.Return_staff(Form2.ComboBox1),Edit1.Text,Edit2.Text);
