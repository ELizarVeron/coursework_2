...
type
  ...
  public
    ...
    class var Records: TObjectList<TAgent>;
    // тут должн ыбыть все записи из БД!!!!!!!нн зависсимо от фильтрации
  protected
  private
  end;

var
  For_Manager: TFor_Manager;
...
begin
  ...
  begin
    ...
    ADOQuery2.SQL.Add('Select * from History_of_reliz where ID_Agent = ' +
      agent.ID_.ToString);
    ...
    begin
      ...
      if (x < 366) then
        agent.Count_s_year := agent.Count_s_year + ADOQuery2.Fields[3]
          .AsInteger;
      ADOQuery3.SQL.Clear;
      ADOQuery3.SQL.Add('Select * From Products where Article= ' +
        ADOQuery2.Fields[1].AsString);
      ADOQuery3.Active := true;
      agent.SUMMA := ADOQuery2.Fields[3].AsInteger * ADOQuery3.Fields[1]
        .AsInteger;
      ADOQuery2.Next;
    end;
...
begin
  ADOQuery1.SQL.Clear;
  ADOQuery1.SQL.Add('UPDATE AGENT SET DISCOUNT =' + disqount + ' where ID = ' +
    id.ToString);
  ADOQuery1.ExecSQL;
end;
