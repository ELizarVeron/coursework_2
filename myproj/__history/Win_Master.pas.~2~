...
procedure TForm3.create_filter();
...
function TForm3.Edit_get_sql(): string;
// вспомогательная функция для формирования текста запроса
begin
  var
    inp, x: string;
  inp := Edit1.Text;
...
procedure TForm3.FiltrChange(Sender: TObject);
// событие на изменение фильтрации
begin // формируем sql запрос
  var
    item, x: string;
  ...
  ADOQuery1.SQL.Clear;
  array_of_agents := Form3.add_data_for_manager(x);
  // передаем полученный запрос в функцию загрухки данных
